...
int lcd_init(struct device *dev)
{
	...
	gpio_dir(LCD_RS,GPIO_DIR_OUT,dev);
	gpio_dir(LCD_RW,GPIO_DIR_OUT,dev);
	gpio_dir(LCD_EN,GPIO_DIR_OUT,dev);
	gpio_dir(LCD_D4,GPIO_DIR_OUT,dev);
	gpio_include <linux/delay.h>
#include "lcd_platform_driver.h"
#include "lcd.h"
#include "gpio.h"
...
int lcd_init(struct device *dev)
{
	...
	gpio_dir(LCD_EN,GPIO_DIR_OUT,dev);
	gpio_dir(LCD_D4,GPIO_DIR_OUT,dev);
	gpio_dir(LCD_D5,GPIO_DIR_OUT,dev);
	gpio_dir(LCD_D6,GPIO_DIR_OUT,dev);
	gpio_dir(LCD_D7,GPIO_DIR_OUT,dev);
...
void lcd_display_return_home(struct device *dev)
{
	...
	/* Check page number 24 of datasheet return home command execution wait time is 		around 2ms */
	mdelay(2);
}
...
{
	lcd_send_command(LCD_CMD_DIS_CLEAR,dev);
	/* check page number 24 of datasheet. display clear command execution wait time is around 2ms */
	mdelay(2);
}
